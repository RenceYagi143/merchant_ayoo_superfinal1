...
export default function StoreSetupPage() {
  ...
  useEffect(() => {
    ...
    // If user has already completed store setup, redirect to dashboard
  ...
  // Update store setup to set the completion flag and save to user properties
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update user with all store information AND mark setup as completed
      await updateUser({
        ...
        logoUrl: logoUrl,
        storeOpen: true,
        storeSetupCompleted: true, // Mark setup as completed
      })

      // Also save to StoreInfo table for dashboard display
      await BackendlessService.saveStoreInfo({
        merchantId: merchantId,
        storeName: formData.storeName,
        storeType: formData.storeType === "Other" ? formData.customStoreType : formData.storeType,
        description: formData.description,
  ...
  // Don't render if already completed setup (will redirect)
  if (user.storeSetupCompleted) {
    return null
  }
      ...
      <div className="container mx-auto max-w-2xl py-8">
        <Card className="border-pink-200 shadow-xl">
          <CardHeader className="text-center">
            ...
            <CardDescription>Complete your store setup to start managing your business</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <Button type="submit" className="w-full bg-pink-600 hover:bg-pink-700" disabled={loading}>
                {loading ? "Setting Up Your Store..." : "Complete Store Setup"}
              </Button>
            </form>
          </CardContent>
        </Card>
